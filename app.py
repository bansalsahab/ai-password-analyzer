...
def load_rockyou_dataset():
    ...
    try:
        ...
        print(f"Pattern analysis on sample of {min(500000, len(passwords))} passwords: {common_patterns}")
...
def analyze_password():
    ...
    improved_password, improvement_reason = suggest_improved_password(password, vulnerabilities)
...
def identify_patterns(password):
    ...
    if re.search(r'(abc|bcd|cde|def|efg|fgh|ghi|hij|ijk|jkl|klm|lmn|mno|nop|opq|pqr|qrs|rst|stu|tuv|uvw|vwx|wxy|xyz|012|123|234|345|456|567|678|789|890)', password, re.IGNORECASE):
    ...
    if re.search(r'(qwert|asdfg|zxcvb|12345|09876|qazws|wsxed|edcrf|rfvtg)', password, re.IGNORECASE):
...
def calculate_score(password, entropy, patterns, in_rockyou):
    ...
    char_set_score = 0
    if re.search(r'[a-z]', password): char_set_score += 2.5
    if re.search(r'[A-Z]', password): char_set_score += 2.5
    if re.search(r'[0-9]', password): char_set_score += 2.5
    if re.search(r'[^a-zA-Z0-9]', password): char_set_score += 2.5
    score += char_set_score
    
    # Penalize for patterns (max -30 points)
    pattern_penalty = 0
    if patterns.get('dictionary_word'): pattern_penalty += 5
    if patterns.get('sequential_chars'): pattern_penalty += 5
    if patterns.get('repeated_chars'): pattern_penalty += 5
    if patterns.get('keyboard_pattern'): pattern_penalty += 10
    if patterns.get('numbers_only'): pattern_penalty += 15
    if patterns.get('letters_only'): pattern_penalty += 10
    if patterns.get('number_suffix'): pattern_penalty += 3
    if patterns.get('special_suffix'): pattern_penalty += 2
    if patterns.get('capital_word_number'): pattern_penalty += 5
    if patterns.get('year'): pattern_penalty += 5
    if patterns.get('date_format'): pattern_penalty += 8
...
def estimate_crack_time(password, entropy):
    ...
    if password in common_passwords:
        guesses = 1000  # Assume it's within the first 1000 attempts of a dictionary attack
    else:
...
def suggest_improved_password(password, vulnerabilities):
    ...
    if "Common Password" in vulnerabilities:
        return generate_strong_password(), "This is a completely new password that follows best practices for security. It's not found in common password databases and has high entropy."
    
    # Add length if too short
    if "Too Short" in vulnerabilities:
        additional_chars = "".join(random.choices(string.ascii_letters + string.digits + string.punctuation, k=10-len(password)))
        improved += additional_chars
        changes_made.append(f"Added {len(additional_chars)} characters to increase length")
    ...
    if "Sequential Characters" in vulnerabilities:
        for seq in ["abc", "bcd", "cde", "def", "123", "234", "345", "456"]:
            if seq.lower() in improved.lower():
                replacement = "".join(random.choices(string.ascii_letters + string.digits, k=3))
                improved = improved.replace(seq, replacement)
                changes_made.append(f"Replaced sequential pattern '{seq}' with '{replacement}'")
    ...
    if "Keyboard Pattern" in vulnerabilities:
        for pattern in ["qwert", "asdfg", "zxcvb", "12345"]:
            if pattern.lower() in improved.lower():
                ...
                changes_made.append(f"Replaced keyboard pattern with unpredictable characters")
    ...
    if "Number Suffix" in vulnerabilities:
        ...
        if match:
            ...
            new_suffix = ""
            for digit in suffix:
                new_suffix += digit + random.choice(string.ascii_letters + string.punctuation)
    ...
    if "Year Pattern" in vulnerabilities:
        for year in ["2022", "2023", "2024", "1990", "1991", "1992", "1993", "1994", "1995"]:
            if year in improved:
                replacement = "".join(random.choices(string.ascii_letters + string.digits + string.punctuation, k=4))
                improved = improved.replace(year, replacement)
                changes_made.append(f"Replaced year {year} with unpredictable characters")
    ...
    if not changes_made or improved == password:
        ...
        suffix = "".join(random.choices(string.ascii_letters + string.digits + "!@#$%^&*()-_=+", k=max(8, original_len)))
        improved = prefix + suffix
        changes_made = ["Created a more complex variation while preserving the beginning for memorability"]
    
    # Create explanation
    reason = "This improved password addresses the vulnerabilities by: " + ", ".join(changes_made) + "."
...
def generate_ai_analysis(password, patterns, in_rockyou, entropy):
    ...
    if patterns.get('dictionary_word'):
        pattern_analysis.append("Contains recognizable words that make it vulnerable to dictionary attacks")
    
    if patterns.get('sequential_chars'):
        pattern_analysis.append("Contains sequential characters (like 'abc' or '123') that reduce complexity")
    
    if patterns.get('repeated_chars'):
        pattern_analysis.append("Contains repeated characters that reduce entropy")
    
    if patterns.get('keyboard_pattern'):
        pattern_analysis.append("Contains keyboard patterns that are among the first patterns attackers try")
    
    if patterns.get('numbers_only'):
        pattern_analysis.append("Consists of only numbers, which drastically limits the possible combinations")
    
    if patterns.get('letters_only'):
        pattern_analysis.append("Contains only letters, missing the extra security from numbers and special characters")
    
    if patterns.get('number_suffix'):
        pattern_analysis.append("Ends with numbers, a pattern used in over 30% of passwords")
    
    if patterns.get('year'):
        pattern_analysis.append("Contains a year, which is highly predictable")
    
    if patterns.get('date_format'):
        pattern_analysis.append("Contains a date pattern, which reduces possible combinations significantly")
    
    if patterns.get('leetspeak'):
        pattern_analysis.append("Uses leetspeak (replacing letters with numbers/symbols), which is a known pattern that attackers check")
    
    pattern_html = ""
    if pattern_analysis:
        pattern_html = "<ul>" + "".join([f"<li>{p}</li>" for p in pattern_analysis]) + "</ul>"
...
def calculate_pattern_data(password):
    ...
    length = len(password)
    char_types = {
        'lowercase': len(re.findall(r'[a-z]', password)) / length * 100 if length else 0,
        'uppercase': len(re.findall(r'[A-Z]', password)) / length * 100 if length else 0,
        'digits': len(re.findall(r'[0-9]', password)) / length * 100 if length else 0,
        'special': len(re.findall(r'[^a-zA-Z0-9]', password)) / length * 100 if length else 0
    }
    ...
    if password in common_passwords:
        ...
        brute_force_strength = min(100, (charset_size * math.log2(max(1, len(password)))) / 7)
    ...
    advanced_metrics = {
        'entropy_per_char': calculate_entropy(password) / len(password) if len(password) > 0 else 0,
        'char_variety_ratio': len(set(password)) / len(password) if len(password) > 0 else 0,
        'sequential_ratio': len(re.findall(r'(abc|bcd|cde|def|123|234|345|456)', password, re.IGNORECASE)) / len(password) if len(password) > 0 else 0,
        'symbol_density': len(re.findall(r'[^a-zA-Z0-9]', password)) / len(password) if len(password) > 0 else 0
    }
